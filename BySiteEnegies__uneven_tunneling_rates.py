...
for gamma23 in gamma23_set:
    ...
    for eR in Energy_1:
        for eL in Energy_2:
            ...
            Gammaij_normalized[0][1] = Modified_Fermi_Function(gamma01 + gammaDD(vR, 0, tR)/T, V/2 - site_1) * gammaDD(vR, 0, tR)
            Gammaij_normalized[1][0] = Modified_Fermi_Function(gamma01 + gammaDD(vR, 0, tR)/T, -V/2 + site_1) * gammaDD(vR, 0, tR)

            Gammaij_normalized[2][3] = Modified_Fermi_Function(gamma23 + gammaDD(vR, 1, tR)/T, V/2 - site_1 - U) * gammaDD(vR, 1, tR)
            Gammaij_normalized[3][2] = Modified_Fermi_Function(gamma23 + gammaDD(vR, 1, tR)/T, -V/2 + site_1 + U) * gammaDD(vR, 1, tR)

            # left dot transitions
            Gammaij_normalized[0][2] = Modified_Fermi_Function(gamma02 + gammaDD(vL, 0, tL)/T, - site_2) * gammaDD(vL, 0, tL)
            Gammaij_normalized[2][0] = Modified_Fermi_Function(gamma02 + gammaDD(vL, 0, tL)/T, + site_2) * gammaDD(vL, 0, tL)

            Gammaij_normalized[1][3] = Modified_Fermi_Function(gamma13 + gammaDD(vL, 1, tL)/T, - site_2 - U) * gammaDD(vL, 1, tL)
            Gammaij_normalized[3][1] = Modified_Fermi_Function(gamma13 + gammaDD(vL, 1, tL)/T, + site_2 + U) * gammaDD(vL, 1, tL)
